	...
	class WrappedComponent extends React.Component {
		...
		componentWillReceiveProps( nextProps ) {
			if ( ! nextProps._data.posts && this.props._data.archiveId !== nextProps._data.archiveId ) {
				//janicak: using new action to ensure new resolved id is passed to handle.fetchArchive
				//nextProps._actions.onLoad();
				nextProps._actions.onLoadId(nextProps._data.archiveId, nextProps );
			}
		}
	...
	const mapDispatchToProps = ( dispatch, props ) => {
		...
		return {
			_actions: {
				...
				//janicak: adding action to ensure new resolved id is passed to handle.fetchArchive
				onLoadId: (id, props) => dispatch( handler.fetchArchive( resolve(id, props))),
